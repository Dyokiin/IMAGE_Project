...
bool QTree::isViewed(){
    ...
    Vec3 p1(this->area.x1, 255/10 -25, this->area.y1);
    Vec3 p2(this->area.x2, -25, this->area.y2);
    Vec3 p3(this->area.x1, 0, this->area.y2);
    Vec3 p4(this->area.x2, 0, this->area.y1);

    if(frustrum.pointInFrustum(p1)==1 || frustrum.pointInFrustum(p2)==1 || frustrum.pointInFrustum(p3)==1 || frustrum.pointInFrustum(p4)==1){return true;}
...
void QTree::display(){
    // std::cout << "(" << this->area.x1 << "," << this->area.y1 <<")(" <<
    //                     this->area.x2 << "," << this->area.y2 << ") " <<
    //                     this->nW << ", " << this->nE << ", " << this->sE << ", " << this->sW << std::endl;
    if(this->isViewed()){
